...
class MaOEACSS(Algorithm):
    ...
    def _sequential_elimination(self, fit: torch.Tensor, zmin: torch.Tensor, n_required: int) -> torch.Tensor:
        ...
        for _ in range(n_to_remove):
            ...
            diff = con[i] - con[j]
            remove_i = torch.where(diff > self.t, torch.tensor(True, device=fit.device), 
                                   torch.where(con[j] - con[i] > self.t, torch.tensor(False, device=fit.device), 
    ...
    def step(self) -> None:
        ...
        angle_current = torch.where(torch.eye(self.pop_size, device=self.pop.device, dtype=torch.bool), 
                                    torch.tensor(1e9, device=self.pop.device), angle_current)
        ...
        final_idx = torch.where(torch.rand(self.pop_size, device=self.pop.device) < prob, 
                                selected_idx, 
