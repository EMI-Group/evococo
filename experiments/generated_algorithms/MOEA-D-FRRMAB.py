...
class MOEADFRRMAB(Algorithm):
    def __init__(self, pop_size: int, n_objs: int, lb: torch.Tensor, ub: torch.Tensor, T: int = 20, delta: float = 0.9, nr: int = 2, window_size: int = None, **kwargs):
        super().__init__()
        device = lb.device
        self.n_objs = n_objs
        self.lb = lb
    ...
    def step(self) -> None:
        ...
        I = torch.cat([boundary_indices, tour_idx])
        I_size = I.numel()
        ...
        for _ in range(5):
            ...
            neighbor_pool = self.neighbors[I]
            ...
            offspring = self._apply_four_de(parents, op_indices, self.pop[I])
            ...
            P_to_update = self.neighbors[I] # [I_size, T]
