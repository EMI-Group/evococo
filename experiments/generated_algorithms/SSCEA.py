...
class SSCEA(Algorithm):
    ...
    def step(self) -> None:
        ...
        final_off = torch.where(condition, 
                                off_pop * self.cv_mask + parents * (~self.cv_mask), 
                                off_pop * self.dv_mask + parents * (~self.dv_mask))
        ...
        # I_ij = max(f_i - f_j)
        I = torch.max(f_norm.unsqueeze(1) - f_norm.unsqueeze(0), dim=-1).values
        C = torch.max(torch.abs(I))
        
        # Initial Fitness F
        # F_i = sum_{j!=i} -exp(-I_ji / (C*0.05))
        # Note: I[j, i] is f_norm[j] - f_norm[i]
        kernel = torch.exp(-I / (C * 0.05 + 1e-6))
