...
class GWASFGA(Algorithm):
    ...
    def step(self) -> None:
        ...
        mating_pool = tournament_selection_multifit(
            self.pop_size, 
            [self.front_no.float(), -self.crowd_dis], 
            tournament_size=2
        )
...
def _gwasf_sort(fit: torch.Tensor, vectors: torch.Tensor, utop: torch.Tensor, nadir: torch.Tensor, N: int) -> tuple[torch.Tensor, torch.Tensor]:
