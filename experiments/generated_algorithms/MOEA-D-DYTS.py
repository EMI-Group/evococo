...
class MOEADDYTS(Algorithm):
    ...
    def _vectorized_de_op(self, op_idx: torch.Tensor, p1: torch.Tensor, p2: torch.Tensor, p3: torch.Tensor, p4: torch.Tensor, p5: torch.Tensor) -> torch.Tensor:
    ...
    def step(self) -> None:
        ...
        I = torch.cat([boundary_indices, other_indices])
        ...
        neighbor_parents = torch.gather(self.B[I], 1, rand_neighbor)
        ...
        for j in range(batch_size):
            ...
            if torch.any(replace_mask):
                ...
                # Take at most nr
                num_replace = torch.minimum(torch.tensor(self.nr, device=device), torch.tensor(replace_indices.shape[0], device=device))
