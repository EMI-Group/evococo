...
class eMOEA(Algorithm):
    ...
    def step(self) -> None:
        ...
        pop_dom_off = (self.fit.unsqueeze(1) <= off_fit.unsqueeze(0)).all(-1) & \
                      (self.fit.unsqueeze(1) < off_fit.unsqueeze(0)).any(-1)
        ...
        off_dom_pop = (off_fit.unsqueeze(1) <= self.fit.unsqueeze(0)).all(-1) & \
                      (off_fit.unsqueeze(1) < self.fit.unsqueeze(0)).any(-1)
        ...
        self.archive_pop, self.archive_fit, self.archive_mask = _update_archive(
            self.archive_pop, self.archive_fit, self.archive_mask,
            off_pop, off_fit, self.epsilon
        )
...
def _update_archive(arc_pop, arc_fit, arc_mask, off_pop, off_fit, epsilon):
    ...
    # Grid Dominance
    dom = (G.unsqueeze(1) <= G.unsqueeze(0)).all(-1) & \
          (G.unsqueeze(1) < G.unsqueeze(0)).any(-1)
