...
class MOEAD_DRA(Algorithm):
    ...
    def step(self) -> None:
        ...
        I = torch.cat([boundary_indices, winners_idx])
        ...
        for _ in range(5):
            ...
            p1_idx = torch.where(sel_mask, self.B[I, r1], rp1)
            p2_idx = torch.where(sel_mask, self.B[I, r2], rp2)
            
            # Offspring Generation (DE/rand/1)
            # Offspring = Population(i) + F * (Population(P1) - Population(P2))
            F = 0.5
            CR = 1.0
            offspring = self.pop[I] + F * (self.pop[p1_idx] - self.pop[p2_idx])
            ...
            for j in range(num_I):
                ...
                perm = torch.randperm(num_better, device=device) if num_better > 0 else torch.empty(0, dtype=torch.long, device=device)
        ...
        if self.gen_counter >= 10:
            ...
            # Piecewise Update
            self.pi = torch.where(delta > 0.001, 
                                  torch.ones_like(delta), 
                                  (0.95 + 0.05 * delta / 0.001) * self.pi)
