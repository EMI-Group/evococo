        ...
        if num_promising <= N:
            ...
            for _ in range(num_p):
                ...
                if torch.sum(active_p.to(torch.int32)) > N:
                    ...
                    row_min = torch.min(torch.where(active_p.unsqueeze(0) & active_p.unsqueeze(1), 
                                                   temp_imatrix, torch.tensor(float('inf'), device=device)), dim=1).values
