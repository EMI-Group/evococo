...
def _ibea_selection(pop, fit, K, kappa):
    ...
    I = torch.max(diff, dim=-1)[0]
    C = torch.max(torch.abs(I))
    
    # Initial Fitness
    fitness = torch.sum(-torch.exp(-I / (C * kappa + 1e-6)), dim=0)
    ...
    for _ in range(N - K):
        ...
        fitness += torch.exp(-I[worst, :] / (C * kappa + 1e-6))
...
def _pc_selection(pop, fit, K):
    ...
    if torch.sum(mask_rank1) <= K:
        ...
        new_pop, new_fit, _, _ = nd_environmental_selection(pop, fit, K)
