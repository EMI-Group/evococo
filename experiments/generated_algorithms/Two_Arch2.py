...
class Two_Arch2(Algorithm):
    ...
    def _update_ca(self, combined_pop, combined_fit):
        ...
        I = torch.max(norm_fit.unsqueeze(1) - norm_fit.unsqueeze(0), dim=-1).values

        # Fitness F
        C = torch.max(torch.abs(I), dim=0).values
        # F_i = sum_{j} -exp(-I(j,i) / (C_i * 0.05))
        # Note: MATLAB uses I(j,i) in the sum for F_i
        F = torch.sum(-torch.exp(-I / (C.unsqueeze(0) + 1e-6) / 0.05), dim=0) + 1.0
        ...
        for _ in range(num_to_remove):
            ...
            F = F + torch.exp(-I[worst, :] / (C[worst] + 1e-6) / 0.05)
