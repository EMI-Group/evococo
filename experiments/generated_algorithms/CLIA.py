...
from evox.utils import clamp, randint, lexsort
...
class CLIA(Algorithm):
    ...
    def _cascade_clustering_selection(self, combined_pop, combined_fit):
        ...
        intra_rank = torch.arange(len(pi), device=pi.device) - torch.where(diff == 1, torch.arange(len(pi), device=pi.device), torch.tensor(0, device=pi.device)).cummax(dim=0)[0]
