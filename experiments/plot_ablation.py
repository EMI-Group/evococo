...
# Injected Benchmark Harness (same as evaluation/benchmark.py)
BOILERPLATE = """
...
def classify_error(code_path):
    """
    Reruns the generated code with the benchmark harness to capture and classify failure root causes.
    """
    try:
        ...
        if "shape" in err_msg or "size" in err_msg or "dimension" in err_msg or "matmul" in err_msg or "broadcast" in err_msg:
            ...
            # Note: EvoX API misuse often manifests as attribute/type errors on states
...
def main():
    configs = {
        "Full EvoCoCo": "./experiments/results_full",
        "w/o Architect": "./experiments/results_no_architect",
        "w/o Runtime Fixer": "./experiments/results_no_runtime_fixer",
        "w/o RAG Rules": "./experiments/results_no_rag",
        "w/o Multi-Branch Selection": "./experiments/results_single_branch",
    }
    ...
    for name, path in configs.items():
        ...
        # Detailed error classification for observations
        shape_mismatches = 0
        evox_api_misuses = 0
    ...
    print("\\begin{table*}[t]")
    print("  \\centering")
    print("  \\caption{Ablation analysis of EvoCoCo components.}")
    ...
    report_file = "./experiments/benchmark_results_final/ablation_table.md"
    os.makedirs(os.path.dirname(report_file), exist_ok=True)
    
    with open(report_file, "w", encoding="utf-8") as f:
        f.write("# Ablation Experiment Observations & Metrics\n\n")
    ...
    for i, (name, val) in enumerate(stats.items()):
        ...
        ax.bar(x + (i - 2) * width, scores, width, label=name)

    ax.set_ylabel("Percentage (%)")
    ax.set_title("Ablation Study: Success Rates on Different Pipeline Stages")
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plot_out = "./experiments/ablation_results.png"
