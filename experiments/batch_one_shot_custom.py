...
from one_shot_custom import one_shot_translate, MODEL_NAME
            ...
            try:
                ...
                if python_code and "class" in python_code:
                    ...
                    with open(out_path, "w", encoding="utf-8") as f:
                        f.write(python_code)
                    print(f"[Success] {out_name} saved on attempt {attempt+1} (Latency: {metrics.get('latency', 0.0):.2f}s, Tokens: {metrics.get('total_tokens', 0):,}).")
                    stats["success"] += 1
                    return
                else:
...
async def main():
    parser = argparse.ArgumentParser(description="Batch One-Shot MATLAB to Python Translation (Custom-aware)")
    parser.add_argument("--input_dir", type=str, default="experiments/evococo_input_non_platemo", help="Directory containing MATLAB files")
    parser.add_argument("--output_dir", type=str, default="experiments/evococo_non_platemo_results_oneshot", help="Directory to save generated Python files")
