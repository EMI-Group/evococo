...
class CMOEA_MS(Algorithm):
    ...
    def step(self) -> None:
        ...
        if self.op_type == 1:
            ...
            off_pop = parents + 0.5 * (self.pop[torch.randint(0, N, (N,), device=device)] - self.pop[torch.randint(0, N, (N,), device=device)])
        ...
        for _ in range(N):
            ...
            temp_dist = torch.where(curr_mask.unsqueeze(0) & curr_mask.unsqueeze(1), dist_mat, torch.full_like(dist_mat, float('inf')))
