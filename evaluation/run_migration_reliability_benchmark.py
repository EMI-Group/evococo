...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing .py scripts to benchmark",
    )
    parser.add_argument(
        "--workers", type=int, default=8, help="Number of concurrent benchmark workers"
    )
    ...
    py_files = [
        f for f in os.listdir(args.dir) if f.endswith(".py") and f != "zero_shot.py"
    ]
    ...
    print("=" * 90)
    print(
        f"{'File':<30} | {'Syntax':<6} | {'Static':<6} | {'Exec':<6} | {'Optim':<6} | {'IGD':<8} | {'Time(s)'}"
    )
    print("-" * 90)
    ...
    async def evaluate_single_file(py_file):
        ...
        async with sem:
            ...
            static_pass = False
            if syntax_pass:
                static_pass, _ = await check_syntax_with_ruff(
                    code, session_id=f"bench_{uuid.uuid4().hex[:6]}"
                )
            ...
            if syntax_pass:
                ...
                if not re.search(
                    r"""if\s+__name__\s*==\s*["']__main__["']\s*:""", code
                ):
                ...
                report = await execute_code_trial(
                    modified_code,
