%% PlatEMO computational scalability benchmark: population scaling
clc;
clear;
...
if smoke_test
    ...
    num_var = 12;
    generations = 2;
    fprintf('Smoke-test mode: AGE-MOEA, DTLZ3, population 20.\n');
end
...
csv_file = fullfile(result_dir, 'platemo_population_scaling.csv');
...
if ~exist(csv_file, 'file')
    ...
    if fid ~= -1
        fprintf(fid, 'Algorithm,PopSize,AvgTime_s,AvgIGD,RawTimes,RawIGDs\n');
        fclose(fid);
    else
...
for algidx = 1:length(algorithm_names)
    ...
    for pidx = 1:length(pop_sizes)
        ...
        if exist(csv_file, 'file')
            ...
            if fid ~= -1
                ...
                for i = 1:length(fileData{1})
                    line = fileData{1}{i};
                    if startsWith(line, sprintf('%s,%d,', algname, N))
                        skip = true;
                        break;
                    end
                end
        ...
        if skip
            fprintf('Skipping completed: %s | PopSize = %d\n', algname, N);
            continue;
        end

        [isSupported, unsupportedReason] = isAlgorithmProblemSupported(algname, problemFcn, num_obj, num_var);
        if ~isSupported
            for next_pidx = pidx:length(pop_sizes)
                nextN = pop_sizes(next_pidx);
                if ~hasCsvPrefix(csv_file, sprintf('%s,%d,', algname, nextN))
                    fid = fopen(csv_file, 'a');
                    fprintf(fid, '%s,%d,NaN,NaN,"[UNSUPPORTED_PROBLEM]","[UNSUPPORTED_PROBLEM]"\n', algname, nextN);
                    fclose(fid);
                end
            end
        ...
        for rep = 1:repeats
            futures{rep} = parfeval(@runPlatemoRepeat, 3, alg, problemFcn, num_obj, num_var, N, maxFE);
        end
        ...
        if success_runs > 0 || any(timeout_flags)
            ...
            fid = fopen(csv_file, 'a');
            fprintf(fid, '%s,%d,%.4f,%.4f,"%s","%s"\n', algname, N, avg_time, avg_igd, raw_times_str, raw_igds_str);
            fclose(fid);
        end

        if any(timeout_flags) && skip_larger_after_timeout
            for next_pidx = pidx+1:length(pop_sizes)
                nextN = pop_sizes(next_pidx);
                if ~hasCsvPrefix(csv_file, sprintf('%s,%d,', algname, nextN))
                    fid = fopen(csv_file, 'a');
                    fprintf(fid, '%s,%d,NaN,NaN,"[SKIPPED_AFTER_TIMEOUT]","[SKIPPED_AFTER_TIMEOUT]"\n', algname, nextN);
                    fclose(fid);
                end
            end
            fprintf('  >> Timeout reached for %s at pop_size=%d. Larger population sizes are skipped.\n', algname, N);
            break;
        end
    end
end
